...
Public Class Cart
    ...
    Public Sub SetProperties(source As DataRow)
        ...
        item_name.Text = source("item_name")
        price.Text = source("price")
        quantity.Text = source("quantity")

        If source("image_dir") IsNot DBNull.Value Then itemImage.Image = Image.FromFile(source("image_dir"))
    ...
    Private Sub btnSelect_Clicked(sender As Object, e As EventArgs)
        ...
        Select Case btnSelect.ForeColor
            ...
            Case Color.LightCoral
                ...
                delete_items &= $"DELETE FROM `order_has_product` WHERE (`order_id` = '{cart_item("id")}'); "
                delete_items &= $"DELETE FROM `order` WHERE (`id` = '{cart_item("id")}'); "

                transactionQuery($"[Item: {cart_item("item_name")}] Removed from order.") = delete_items
            ...
            Case Color.Yellow
                ...
                Dim delete_items As String = $"DELETE FROM `cart` WHERE `cart`.`id` = {cart_item("id")};"

                transactionQuery($"[Item: {cart_item("item_name")}] Removed from cart.") = delete_items
                p.Dispose()
            ...
            Case Color.Black
                ...
                MakeItemSummary(_cartpage.order_summary, index)
                SetCost(1)
                ...
                VALUES
                (LAST_INSERT_ID(), {cart_item("product_id")}, {quantity.Text}, {cart_item("cost_per_item") + 150}, 'pending');
                "
                query &= $"DELETE FROM `cart` WHERE `cart`.`id` = {cart_item("id")};"


                transactionQuery($"{cart_item("id")}") = query
            ...
            Case Color.Red
                ...
                transactionQuery.Remove(cart_item("id"))
        End Select
    ...
    Public Sub MakeItemSummary(parent As Control, id As Integer)
    ...
    Public Function CartItem() As Panel
        ...
        btnSelect.BackgroundImage = Global.ProjectTesting_072423.My.Resources.Resources.icons8_unchecked_checkbox_30
        addQuant.BackgroundImage = Global.ProjectTesting_072423.My.Resources.Resources.icons8_add_new_96
        subQuant.BackgroundImage = Global.ProjectTesting_072423.My.Resources.Resources.icons8_indeterminate_checkbox_96
        itemImage.Image = Global.ProjectTesting_072423.My.Resources.Resources.icons8_no_image_30
